...
```bash
# 启动本地服务（Windows，项目根有 _serve.bat 等价）
.venv/Scripts/python.exe -m src.server   # http://127.0.0.1:8642
...
# （打包/比对用子集，全量仅供校验；改 indexer.CHARS 后需重跑）
.venv/Scripts/python.exe tools/subset_fonts.py

# 重建字形索引（改字体白名单/阈值后必须重建，否则比对用旧索引）
.venv/Scripts/python.exe -m src.indexer

# 生成模拟截图测试集（依赖 src/render.render_char）
.venv/Scripts/python.exe tools/gen_testset.py
```

无 lint / format / pytest 配置，纯 Python + NumPy + Pillow，标准库 HTTP 服务，无框架。

**分发形态：源码运行（已放弃 exe 打包）**——对外交付用「.venv + 源码」或压缩包方式，双击 `_serve.bat` / `_start_hidden.vbs` 启动后自动打开浏览器。PyInstaller 打包仅作备用手段（曾出现对方机器 WebView2/杀软兼容问题），`FontCop.spec` 保留但不再维护发布。

## 架构

- `src/server.py` — HTTP 服务 + API（/api/match 手动框选、/api/auto OCR 自动）。打包模式下资源在 `_MEIPASS`，数据写入 exe 旁 `FontCop_data/`（用 `sys.frozen` 分支处理，别删）。`start_server()` 带单实例保护（端口被占时只开浏览器界面并退出，不重复起服务）；启动就绪后自动打开默认浏览器。
...
- `src/edt.py` — 纯 NumPy 精确欧氏距离变换（Felzenszwalb 抛物线，O(n)），替代 scipy.ndimage.distance_transform_edt（逐像素一致，已移除 scipy 依赖——这是 exe 从 229MB 减到 117MB 的关键之一）。
- `src/auto.py` — RapidOCR 自动模式（可选依赖，未安装时前端退回手动）。
...
- **子集化字体**：比对/打包只用 `fonts/subset/`（12 款共 ~4MB，`src/render.glyph_files()` 优先子集、缺则回退 `fonts/files/` 全量）。全量 `fonts/files/` 192MB 仅本地校验用、不打包。`inter`/`jetbrains-mono`/`dejavu-sans` 缺源文件，未子集化、空字形不参与比对。
...
- **PyInstaller 对"无匹配 glob"直接报错**：`FontCop.spec` 里的 datas 是手工展开的（缺文件时跳过），新增打包资源要同步改 spec。
- **`.bak` 文件是历史快照**，不要读取或基于其改动；根目录日志/临时文件（`*.log`、`nul`、`_tmp_*`、`_probe*`）均已 gitignore。
- 打包内 rapidocr 的 config.yaml/onnx 必须整体打入 spec，删减会导致运行时 OCR 崩溃。
